...
# Global variables
url = "http://192.168.1.99:3000/voting" #api calls (voting) /voting (interaction) /modify
station_id = 'v1a' #voting station ids: [v1a,v1b,v1c,v2a,v2b,v2c] interaction stations: [int1, int2]
type = "vote" #types [vote, interacton]
tag = ""
stat=0;
...
try:
    ...
    def insert_history(nfc, type, station_id, strip):
        payload = {'nfc': nfc, 'type': type, 'station_id': station_id}
    	try:
    ...
    while True:
        ...
        if (tag != nfc): #if a new nfc then proceed
            ...
            print("new card lets send it")
            insert_history(nfc, type, station_id, strip)
            tag = nfc #needed to ensure we dont read the tag muliple times
