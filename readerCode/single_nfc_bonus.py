...
try:
    ...
    def update_status(mode, nfc, station_id, strip):
        payload = {'mode': mode, 'nfc': nfc, 'station_id': station_id}
        try:
            r = requests.get(url, params=payload)
        except:
            errCode(strip, Color(0, 0, 255))
    ...
    while True:
        ...
        if (tag != nfc):
            ...
            if stat == 0:
                ...
                update_status(6, nfc, station_id, strip)
                colorWipe(strip, Color(255, 0, 0)) # Green
                check_status(nfc, strip)
		insert_history(nfc, 1, station_id, strip)
            tag = nfc
