...
function setup_environment {
  ...
  URL="http://www.ed.ac.uk/schools-departments/information-services/computing/desktop-personal/wireless-networking/jrs-eduroam/jrs-eduroam-vistors"
  SUPPORT="IS.Helpline@ed.ac.uk"
if [ ! -z "$DISPLAY" ] ; then
...
if ! ask "This installer will only work properly if you are a member of ${bf}The University of Edinburgh.${n}" "Continue" 1 ; then exit; fi
...
-----BEGIN CERTIFICATE-----

...

2ZcIi9++NijQekRQcZbDYDRbwzmTuZeY5BOfaLr7PSKcEMtkOvfvLJb8
-----END CERTIFICATE-----
...
class EduroamNMConfigTool:
    ...
    def add_connection(self,ssid):
        server_alt_subject_name_list = dbus.Array({'DNS:radius.is.ed.ac.uk'})
        server_name = 'radius.is.ed.ac.uk'
        if self.nm_version == "0.9" or self.nm_version == "1.0":
...
network={
  ...
  identity="${USER_NAME}"
  domain_suffix_match="radius.is.ed.ac.uk"
  phase2="auth=MSCHAPV2"
  password="${PASSWORD}"
}
EOFW
